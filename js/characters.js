 ...
 * 19 Characters | bodyEmoji(게임 스프라이트) + weaponEmoji(무기) 분리
 */

const TYPE_NAMES = {
    physical:'물리', magic:'마법', support:'지원',
    hybrid:'혼합', dark:'암흑', tech:'기계'
};
...
function getCharById(id) { return CHARACTERS.find(c => c.id === id) || null; }
function getCharsByGender(g) { return g==='all' ? CHARACTERS : CHARACTERS.filter(c=>c.gender===g); }
